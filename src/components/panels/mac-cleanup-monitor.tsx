...
export function MacCleanupMonitor() {
  ...
  const run = useCallback(async (id: string, mode: 'audit' | 'dry-run' | 'auto') => {
    ...
    try {
      ...
      setFlash(result.decision.reason + (result.ok ? '' : result.stderr ? ` ${result.stderr}` : ''))
      await fetchData()
    } catch (err: unknown) {
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Realtime CPU, RAM, swap, and disk vs the LaunchAgent thresholds. Auto prune only runs
            when disk is below {snapshot.thresholds.diskEmergencyGb} GB and npm/pnpm are idle.
          </p>
        </div>
