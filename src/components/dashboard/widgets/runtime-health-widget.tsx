...
export function RuntimeHealthWidget({ data }: { data: DashboardData }) {
  const { localOsStatus, mcHealth, memPct, systemStats, cliFleets, isSessionsLoading } = data
    ...
    <div className="panel">
      ...
      <div className="panel-body space-y-3">
        ...
        {cliFleets.filter((fleet) => fleet.kind !== 'gateway').map((fleet) => (
          <HealthRow
            key={fleet.kind}
            label={`${fleet.label} Runtime`}
            value={isSessionsLoading ? 'Loading...' : fleet.health.value}
            status={isSessionsLoading ? 'warn' : fleet.health.status}
          />
        ))}
