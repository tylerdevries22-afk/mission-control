...
export async function runMacCleanup(input: {
  ...
  if (!entry || !entry.triggerable) {
    return {
      ...
      id: input.id,
      mode: input.mode,
      decision: { allowed: false, code: 'unknown_job', reason: 'Automation is not triggerable.' },
      stdout: '',
      stderr: '',
      code: null,
    }
  ...
  try {
    ...
    if (!isAllowlistedBinary(binary, binary, stat)) {
      return {
        ...
        id: input.id,
        mode: input.mode,
        decision: { allowed: false, code: 'unsafe_binary', reason: 'Binary failed owner/mode/symlink checks.' },
        stdout: '',
        stderr: '',
        code: null,
      }
    ...
    return {
      ...
      id: input.id,
      mode: input.mode,
      decision: { allowed: false, code: 'missing_binary', reason: 'Allowlisted binary is not installed.' },
      stdout: '',
      stderr: '',
      code: null,
    }
