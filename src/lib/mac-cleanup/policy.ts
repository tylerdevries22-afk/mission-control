...
export function recommendAction(
  ...
  if (disk && disk.severity === 'critical') {
    if (metrics.npmActive || metrics.pnpmActive) {
      return {
        action: 'wait',
        reason: 'Disk is below the emergency floor, but npm/pnpm are busy. Cache cleanup stays deferred.',
        automationId: null,
      }
    }
    if (metrics.npmActive == null || metrics.pnpmActive == null) {
      return {
        action: 'wait',
        reason: 'Disk is below the emergency floor, but package-tool activity is unknown.',
        automationId: null,
      }
    }
    return {
      action: 'run-storage',
      reason: `Free disk is ${metrics.diskFreeGb} GB (emergency ${thresholds.diskEmergencyGb} GB). Safe npm/pnpm cache prune is allowed.`,
      automationId: 'storage-maintenance',
    }
  }
  ...
  if (disk?.breached) {
    return {
      action: 'watch',
      reason: `Free disk is below ${thresholds.diskLowGb} GB. Auto prune waits for the emergency floor or an idle package-tool window.`,
      automationId: 'storage-maintenance',
    }
  }

  if (swap?.breached || ram?.breached) {
    return {
      action: 'observe',
      reason: 'Memory pressure is high. RAM purge is disabled; the guardian may notify only.',
      automationId: 'resource-guardian',
    }
  }
  ...
  if (cpu?.breached) {
    return {
      action: 'observe',
      reason: 'CPU load is above threshold. Process killing is disabled; observation only.',
      automationId: 'resource-guardian',
    }
  }
