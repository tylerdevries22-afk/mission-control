...
describe('renameClaudeFleetAgentRows', () => {
  it('renames 20x/5x rows onto claude-1/claude-2 and reassigns tasks', () => {
    ...
    const task = db.prepare('SELECT assigned_to FROM tasks WHERE id = 1').get() as { assigned_to: string }
    expect(task.assigned_to).toBe('claude-1')
    const assigned = db.prepare(
      'SELECT agent_name FROM project_agent_assignments ORDER BY agent_name',
    ).all() as Array<{ agent_name: string }>
    expect(assigned.map((row) => row.agent_name)).toEqual(['claude-1', 'claude-2'])
  })
