...
describe('fleet agents', () => {
  ...
  it('canonicalizes leftover 20x/5x aliases onto isolated seats', () => {
    ...
    expect(canonicalFleetAgentName('claude')).toBe('claude-1')
    expect(asFleetAgentName('claude-20x')).toBe('claude-1')
  ...
  it('maps workspaces and account dirs without treating 20x as claude-2', () => {
    ...
    expect(fleetAgentFromWorkspace('/x/workspace-claude-1')).toBe('claude-1')
