...
async function runCodex(sessionId: string, prompt: string, modelId?: string): Promise<string> {
  ...
  try { await runCommand('codex', args, { timeoutMs: 180000 }) } catch { /* read output anyway */ }
...
export async function runSessionContinue(input: {
  ...
  if (input.kind === 'grok') {
    ...
    const result = await runCommand(await resolveExecutable('grok'), args, { timeoutMs: 180000, input: input.prompt })
    return (result.stdout || '').trim() || (result.stderr || '').trim()
  }
  if (input.kind === 'kimi') {
    const args = ['-S', input.sessionId, '-p', input.prompt]
    if (input.modelId) args.push('-m', input.modelId)
    const result = await runCommand(await resolveExecutable('kimi'), args, { timeoutMs: 180000 })
    return (result.stdout || '').trim() || (result.stderr || '').trim()
  }
  const result = await runCommand(getOpenCodeExecutable(), ['run', '--session', input.sessionId, input.prompt], {
    timeoutMs: 180000,
  })
  return (result.stdout || '').trim() || (result.stderr || '').trim()
}
